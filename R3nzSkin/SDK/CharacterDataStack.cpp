...
void CharacterDataStack::push(const char* model, const std::int32_t skin) noexcept
{
	...
	static const auto Push{ reinterpret_cast<FnPush>(cheatManager.memory->base + offsets::functions::CharacterDataStack__Push) };
...
void CharacterDataStack::update(const bool change) noexcept
{
	...
	static const auto Update{ reinterpret_cast<FnUpdate>(cheatManager.memory->base + offsets::functions::CharacterDataStack__Update) };
	Update(this, change);
}
