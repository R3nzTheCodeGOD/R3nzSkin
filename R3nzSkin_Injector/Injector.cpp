	...
	try
	{
		...
		if (std::regex_search(json, tagnameMatch, tagnameRegex))
		{
			...
			if (std::regex_search(json, dateMatch, dateRegex))
			{
				...
				{
					auto date_of_github_release_class = DateTime::ParseExact(date_of_github_release, L"dd.MM.yyyy HH:mm", CultureInfo::InvariantCulture);
					auto date_of_current_release_class = DateTime::ParseExact(date_of_current_release, L"dd.MM.yyyy HH:mm", CultureInfo::InvariantCulture);
